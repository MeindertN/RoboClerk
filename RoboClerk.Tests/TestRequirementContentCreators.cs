...
namespace RoboClerk.Tests
{
    ...
    internal class TestRequirementContentCreators
    {
        ...
        public void TestSetup()
        {
            ...
            dataSources.GetTemplateFile("./ItemTemplates/ASCIIDOC/Requirement.adoc").Returns(File.ReadAllText("../../../../RoboClerk.Core/ItemTemplates/ASCIIDOC/Requirement.adoc"));
        }
        ...
        {
            var sysReq = new SystemRequirement(dataSources, traceAnalysis, config);
            var tag = new RoboClerkTextTag(0, 37, "@@SLMS:SystemRequirement(ItemID=21)@@", true);
            ...
            var softReq = new SoftwareRequirement(dataSources, traceAnalysis, config);
            tag = new RoboClerkTextTag(0, 39, "@@SLMS:SoftwareRequirement(ItemID=55)@@", true);
            dataSources.GetItem("55").Returns(softReqItems[0]);
            content = softReq.GetContent(tag, documentConfig);
            ClassicAssert.IsTrue(Regex.Replace(content, @"\r\n", "\n") == "\n|====\n| soft ID: | 55\n\n| soft Revision: | some\n\n| soft Category: | \n\n| Parent ID: | N/A\n\n| Title: | \n\n| Description: a|        \nDescription\n|====\n");

            var docReq = new DocumentationRequirement(dataSources, traceAnalysis, config);
            tag = new RoboClerkTextTag(0, 44, "@@SLMS:DocumentationRequirement(ItemID=99)@@", true);
