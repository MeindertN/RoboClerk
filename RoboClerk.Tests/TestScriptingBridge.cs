...
namespace RoboClerk.Tests
{
    ...
    public class ScriptingBridgeTests
    {
        ...
        {
            // Arrange
            var bridge = new ScriptingBridge(mockDataSources, mockTraceAnalysis, mockSourceTraceEntity);
            var tag = new RoboClerkTag(0, 40, "@@TEST:ID(param1=value1,param2=value2)@@", true);
            bridge.Tag = tag;
        ...
        public void ScriptingBridge_TagProperty_VERIFIES_DirectAccess()
        {
            // Arrange
            var bridge = new ScriptingBridge(mockDataSources, mockTraceAnalysis, mockSourceTraceEntity);
            var tag = new RoboClerkTag(0, 20, "@@TEST:ID(test=true)@@", true);
        ...
        public void ScriptingBridge_GetAllTagParameterNames_VERIFIES_AllParametersReturned()
        {
            // Arrange
            var bridge = new ScriptingBridge(mockDataSources, mockTraceAnalysis, mockSourceTraceEntity);
            var tag = new RoboClerkTag(0, 35, "@@TEST:ID(alpha=1,beta=2,gamma=3)@@", true);
            bridge.Tag = tag;
        ...
        {
            // Arrange
            var bridge = new ScriptingBridge<RequirementItem>(mockDataSources, mockTraceAnalysis, mockSourceTraceEntity);
            var tag = new RoboClerkTag(0, 39, "@@REQ:ID(priority=high,status=active)@@", true);
            bridge.Tag = tag;
