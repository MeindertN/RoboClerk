...
{
	if (showResults)
		return $"| *File Name* | *Function Name* | *{te.Name} ID* | *Purpose* | *Acceptance* | *Linked {softwareRequirementName}s* | *Linked {softwareSystemTestName}* | *Result* | *Timestamp* \n";
	else
		return $"| *File Name* | *Function Name* | *{te.Name} ID* | *Purpose* | *Acceptance* | *Linked {softwareRequirementName}s* | *Linked {softwareSystemTestName}* \n"; 
}
...
string CreateRows()
{
	...
	foreach( var item in Items)
	{
		...
		if (showResults)
		{
			...
			sb.Append($"| {st.UnitTestFileName} | {st.UnitTestFunctionName} | {st.ItemID} | {st.UnitTestPurpose} | {st.UnitTestAcceptanceCriteria} | {reqs.ToString()} | {tests.ToString()} | {actualResult} | {timestamp} \n\n");
		}
		else
			sb.Append($"| {st.UnitTestFileName} | {st.UnitTestFunctionName} | {st.ItemID} | {st.UnitTestPurpose} | {st.UnitTestAcceptanceCriteria} | {reqs.ToString()} | {tests.ToString()} \n\n");
	}
	return sb.ToString();
}]|====
[csx:CreateTopRow()]
